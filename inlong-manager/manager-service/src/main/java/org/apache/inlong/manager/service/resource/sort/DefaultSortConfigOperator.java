...
public class DefaultSortConfigOperator implements SortConfigOperator {
    ...
    public Boolean accept(String sinkType) {
        return SinkType.SORT_FLINK_SINK.contains(sinkType);
    }

    @Override
    public void buildConfig(InlongGroupInfo groupInfo, InlongStreamInfo streamInfo, StreamSink sinkInfo,
            boolean isStream)
            throws Exception {
        if (isStream) {
        ...
        GroupInfo sortConfigInfo = this.getGroupInfo(groupInfo, streamInfo, sinkInfo);
        String dataflow = OBJECT_MAPPER.writeValueAsString(sortConfigInfo);
        this.addToSinkExt(sinkInfo, dataflow);
    ...
    private GroupInfo getGroupInfo(InlongGroupInfo groupInfo, InlongStreamInfo inlongStreamInfo, StreamSink sinkInfo) {
        ...
        // get sink info
        // Map<String, List<StreamSink>> sinkMap = sinkService.getSinksMap(groupInfo, Collections.singletonList(streamInfo));
        ...
        if (InlongConstants.STANDARD_MODE.equals(groupInfo.getInlongGroupMode())) {
            if (CollectionUtils.isNotEmpty(transformResponses)) {
                ...
                relations = NodeRelationUtils.createNodeRelations(sources, Collections.singletonList(sinkInfo));
            }

            for (int i = 0; i < sources.size(); i++) {
                addAuditId(sources.get(i).getProperties(), sinkInfo.getSinkType(), false);
            }
        } else {
            if (CollectionUtils.isNotEmpty(transformResponses)) {
                ...
                relations = Arrays.asList(NodeRelationUtils.createNodeRelation(sourcesNames, transFormNames),
                        NodeRelationUtils.createNodeRelation(transFormNames,
                                Collections.singletonList(sinkInfo.getSinkName())));
            } else {
                relations = NodeRelationUtils.createNodeRelations(sources, Collections.singletonList(sinkInfo));
            }
        ...
        List<Node> nodes = this.createNodes(sources, transformResponses, Collections.singletonList(sinkInfo), fieldMap);
     ...
     */
    private void addToSinkExt(StreamSink sink, String value) {
        if (sink.getExtList() == null) {
            sink.setExtList(new ArrayList<>());
        }

        SinkExtInfo extInfo = new SinkExtInfo();
        extInfo.setInlongGroupId(sink.getInlongGroupId());
        extInfo.setSinkId(sink.getId());
        extInfo.setKeyName(InlongConstants.DATAFLOW);
        extInfo.setKeyValue(value);

        sink.getExtList().removeIf(ext -> extInfo.getKeyName().equals(ext.getKeyName()));
        sink.getExtList().add(extInfo);
    }
