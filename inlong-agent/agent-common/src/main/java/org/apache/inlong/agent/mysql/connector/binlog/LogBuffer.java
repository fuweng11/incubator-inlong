...
public class LogBuffer {
    ...
    public static final int[] POWERS_10 = {1, 10, 100, 1000, 10000, 100000,
            1000000, 10000000, 100000000, 1000000000};
    public static final int DIG_PER_INT32 = 9;
    public static final int SIZE_OF_INT32 = 4;
    private static final Charset[] encodingsToTry = new Charset[]{StandardCharsets.UTF_8,
            Charset.forName("GBK"), Charset.forName("GB2312"), StandardCharsets.ISO_8859_1};
    private static final Charset[] encodingsGBK = new Charset[]{Charset.forName("GBK"),
            StandardCharsets.UTF_8, Charset.forName("GB2312"), StandardCharsets.ISO_8859_1};
    ...
    public final String getFixString(final int len, String charsetName) {
        ...
        // for charset compatibility
        byte[] chars = new byte[found - from];
        System.arraycopy(buf, from, chars, 0, found - from);

        Charset[] defaultCharsets = encodingsToTry;
        if (charsetName.equalsIgnoreCase("GBK")) {
            defaultCharsets = encodingsGBK;
        }
        for (Charset encodingToTry : defaultCharsets) {
            try {
                String string = decode(chars, encodingToTry);
                position += len;
                return string;
        ...
        throw new IllegalStateException("was not able to encode string using these encodings"
                + Arrays.toString(defaultCharsets));
    }
    ...
    public final String getFullString(final int len, String charsetName) {
        ...
        System.arraycopy(buffer, position, chars, 0, len);
            ...
            if (x != 0) {
                if (mark < pos) {
                    for (int i = DIG_PER_DEC1; i > 0; i--) {
                        final int divisor = POWERS_10[i - 1];
                        final int y = x / divisor;
                        buf[pos++] = ((char) ('0' + y));
                        x -= y * divisor;
                    }
