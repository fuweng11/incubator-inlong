...
package org.apache.inlong.agent.core.dbsync.ha.listener;
...
public class JobRunNodeChangeListener
    ...
    public void childEvent(CuratorFramework client, TreeCacheEvent event)
        ...
        String dbJobId = ConfigDelegate.getDbJobIdFromZKPath(eventPath);
        logger.info("path: {}, eventType: {}, clusterId: {}, dbJobId: {}", event.getData().getPath(),
                event.getType(), clusterId, dbJobId);
        ...
        switch (event.getType()) {
            ...
            case NODE_UPDATED:
                try {
                    ...
                    dispatcher.updateRunJobInfo(dbJobId, jobRunNodeInfo);
                } catch (Exception e) {
            ...
            case NODE_REMOVED:
                try {
                    dispatcher.removeLocalRunJob(dbJobId);
                } catch (Exception e) {
                    logger.error("checkOrStopJob has exception e = {}", e);
                }
                break;
            case CONNECTION_LOST:
                dispatcher.updateZkStats(clusterId, dbJobId, false);
                logger.info("connection lost===============");
                break;
            case CONNECTION_RECONNECTED:
                dispatcher.updateZkStats(clusterId, dbJobId, true);
